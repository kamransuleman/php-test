...
class BookingRepository extends BaseRepository
{
    ...
    {
        $pagenum = ($request->has('page')) ? $request->query('page') : '1';
        $cuser = User::find($user_id);
        $usertype = '';
        $emergencyJobs = [];
        $normalJobs = [];
    ...
    public function jobToData($job)
    {
        ...
        if ($job->certified)
            if ($job->certified == 'both')
                $data['job_for'][] = ['Godkänd tolk', 'Auktoriserad'];
            else if ($job->certified == 'yes')
    ...
    public function endJob($post_data = [])
    {
        ...
        return $job_detail->status != 'started' ?: ['status' => 'success'];
